...
def load_sst(text_field, label_field, batch_size):
    train, dev, test = TabularDataset.splits(path='{}/data/'.format(root), train='Train.tsv',
                                             validation='Valid.tsv', test='Test.tsv', format='csv',
...
model = BiLSTMSentiment(embedding_dim=EMBEDDING_DIM, hidden_dim=HIDDEN_DIM, vocab_size=len(text_field.vocab),
                        label_size=len(label_field.vocab) - 1, use_gpu=USE_GPU, batch_size=BATCH_SIZE)
